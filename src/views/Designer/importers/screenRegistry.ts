...
export const designerScreens: DesignerScreenDefinition[] = [
  ...
  ].filter(([id])=>id!=="karaoke").map(([id,name]) => ({ id:`admin-${id}`, name, target:"admin" as const, route:`/admin?designerSection=${id}&designerPreview=1`, component:`AdminPanel:${id}`, viewport:{width:1440,height:900}, rootSelector:".root" })),
  ...
  ...[["duelo","Duelo de Talentos"],["ftl","Follow the Leader"],["pt","Personal Trainer"],["karaoke","Si lo sabe cante"]].filter(([id])=>id!=="karaoke").map(([id,name])=>({ id:`giant-${id}`, name, target:"giant" as const, route:`/pantalla?designerState=${id}&designerPreview=1`, component:`EscenarioScreen:${id}`, viewport:{width:1920,height:1080}, rootSelector:".screen" })),
  ...[["logo_animado","Logo animado"],["mensaje_app","Mensaje de la app"],["promo","Promoción"],["game_rey","Placa Rey del Orto"],["game_trivia","Placa Desafío Demente"],["game_suma","Placa Sumate"],["game_palabra","Placa Arma la palabra"],["escenario","Placa Escenario"],["break","Pausa"],["cierre","Cierre"],["duelo","Placa Duelo"],["escenario_ftl","Placa Follow the Leader"],["escenario_pt","Placa Personal Trainer"],["escenario_karaoke","Placa Karaoke"]].filter(([id])=>id!=="escenario_karaoke").map(([id,name])=>({ id:`giant-placa-${id}`, name, target:"giant" as const, route:`/pantalla?designerState=placa:${id}&designerPreview=1`, component:`PlacaScreen:${id}`, viewport:{width:1920,height:1080}, rootSelector:".screen" })),
];
